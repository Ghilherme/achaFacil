...
class FavoritesList extends StatelessWidget {
  FavoritesList({Key key, @required this.idDates}) : super(key: key);
  final List<String> idDates;

  @override
  Widget build(BuildContext context) {
    List<DateTime> dt = idDates.map((e) => DateTime.parse(e)).toList();

    Query query = FirebaseFirestore.instance
        .collection('contatos')
        .where('criacao', whereIn: dt)
        .orderBy('avaliacao.geral', descending: true);

    return Scaffold(
        appBar: AppBar(
            title: Text('Favoritos S2'),
            elevation: 0,
            leading: IconButton(
